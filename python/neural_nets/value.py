...
class ValueNN():
    def __init__(self):
        with tf.name_scope('value'):
            ...
            with tf.name_scope('sigmoid'):
                self._sig_weights = tf.Variable(
                    tf.truncated_normal([constants.NUM_CLASSES, 1],
                                        stddev=1.0 / math.sqrt(float(constants.NUM_CLASSES))),
                    name="weights")
                self._sig_biases = tf.Variable(tf.zeros([constants.SLOW_HIDDEN1]), name='biases')
                variable_summaries(self._h1_weights, "sig/weights")
                variable_summaries(self._h1_biases, "sig/biases")
    ...
    def inference(self, boards):
        with tf.name_scope('value'):
            ...
            with tf.name_scope('sigmoid'):
                logits = tf.nn.relu(tf.matmul(logits, self._sig_weights) + self._sig_biases)
                tf.histogram_summary('sig/preactivations', logits)
        return logits
